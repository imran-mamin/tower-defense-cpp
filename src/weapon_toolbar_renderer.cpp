...
WeaponToolbarRenderer::WeaponToolbarRenderer(sf::RenderWindow &renderWindow,
					     GameGrid &gameGrid,
...
/* Helper. */
bool WeaponToolbarRenderer::IsCoordinateInsideTheToolbar(std::uint32_t x, std::uint32_t y) const {
    return x >= gameGrid_.Width() * gameGrid_.TileWidth() &&
	   x <= gameGrid_.Width() * gameGrid_.TileWidth() +
		    WeaponToolbarWidthPx &&
...
const sf::FloatRect WeaponToolbarRenderer::GreenCannonButtonGlobalBounds() const {
...
const sf::FloatRect WeaponToolbarRenderer::MissileLauncher1ButtonGlobalBounds() const {
