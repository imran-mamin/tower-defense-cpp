...
MapInfo ParseMap(const std::string &path) {
	...
	try {
		...
		/*
		 * Make sure the background tile count matches the grid size and the widths and heights match.
		 */
		if (backgroundTiles.size() != mapHeight) {
			throw MapParserError("Column-wise background tile count must be equal to the map height!");
		}
		std::size_t tileTotalCount = 0;
		for (auto row : backgroundTiles) {
			tileTotalCount += row.size();
			if (row.size() != mapWidth) {
				throw MapParserError("Row-wise background tile count must be equal to the map width!");
			}
		}
		...
		for (auto vec : enemyPath_) {
			/* Check that the vectors (only) have start point (a) and the ending point (b). */
			if (vec.size() != 2) {
				throw MapParserError("Error in the enemy path. Each vector must have both a starting point and an ending point and nothing else.");
			}
			/* Check for each point that: that the points have only the x and y values. */
			for (auto point : vec) {
				/* the point has only x and y value, */
				if (point.size() != 2) {
					throw MapParserError("Error in one of the points of one of the enemy vectors. Each point must have its x and y value and nothing else.");
				}
				/* and that the point is within the level bounds. */
				if (point[0] > tileWidth * mapWidth) {
					throw MapParserError(std::string("Erroneous value for x. Value ") + std::to_string(point[0]) + " is not within the level bounds.");
				}
				if (point[1] > tileWidth * mapHeight) {
					throw MapParserError(std::string("Erroneous value for y. Value ") + std::to_string(point[1]) + " is not within the level bounds.");
				}
			}
		}
		// TODO
