...
GameGrid::GameGrid(const MapInfo &mapInfo)
    : obstacleTiles_(mapInfo.obstacleTiles), enemyPath_(mapInfo.enemyPath) {
  width_ = mapInfo.mapWidth;
  height_ = mapInfo.mapHeight;
  tileWidth_ = mapInfo.tileWidth;

  // TODO: Check the enemyPath correctness. It HAS to be at a multiple of the
  // tile size (between two tiles). Throw an error if this isn't the case.
  ...
  for (auto path : mapInfo.enemyPath) {
    ...
    if (path.a.y == path.b.y && path.a.x != path.b.x) {
      ...
      std::uint32_t tileStartX =
          smallerX > 0 ? smallerX - tileWidth_ : smallerX;
      for (; tileStartX <= largerX && tileStartX < width_ * tileWidth_;
           tileStartX += tileWidth_) {
        if (tileStartX < tileWidth_ * width_) {
          /* Note: a.y & b.y are the same. */
          const std::uint32_t currentY = path.a.y;

          if (currentY != 0) {
            TileAtAbsoluteCoordinate(tileStartX, currentY - 1)
                .SetType(TileType::Path);
          }
          TileAtAbsoluteCoordinate(tileStartX, currentY)
              .SetType(TileType::Path);
        }
      }
    }
    /* Vertical line. */
      ...
      std::uint32_t tileStartY =
          smallerY > 0 ? smallerY - tileWidth_ : smallerY;
      for (; tileStartY <= largerY && tileStartY < height_ * tileWidth_;
           tileStartY += mapInfo.tileWidth) {
        if (tileStartY < tileWidth_ * height_) {
          /* Note: a.x & b.x are the same. */
          const std::uint32_t currentX = path.a.x;

          if (currentX != 0) {
            TileAtAbsoluteCoordinate(currentX - 1, tileStartY)
                .SetType(TileType::Path);
          }
          TileAtAbsoluteCoordinate(currentX, tileStartY)
              .SetType(TileType::Path);
        }
      }
    } else {
      throw std::runtime_error(
          "Diagonal enemyPath lines are not currently supported.");
    }
  }

  for (auto xd : obstacleTiles_) {
    TileAtAbsoluteCoordinate(xd.second.first, xd.second.second).occupy();
  }
}

/* Helper method. */
Pos GameGrid::AbsoluteCoordinateToClosestTilePosition(std::uint32_t x,
                                                      std::uint32_t y) {
  /* Note: Without these intermediate steps the compiler will optimize the
   * division plus multiplication operations out. */
