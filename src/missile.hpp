...
class Missile : public Projectile {
 ...
 public:
  Missile(Game* game, int travel_speed, int damage, int radius, Enemy* target,
          Pos position, double exp_radius = 0)
      : Projectile(game, travel_speed, damage, radius, position),
        target_(target),
        explosionRadius_(exp_radius),
        lastDirection_(Pos{target->getPosition().x - position.x,
