...
bool Enemy::isAlive() const { return this->health_ > 0; }
...
int findCurrentPath(const Enemy* e, const std::vector<Vec2D>& path,
  ...
  for (int j = 0; j < (int)path.size(); j++) {
    ...
    // Calculate the difference between ending point and starting point of
    // current Vec2D.
    ...
    if ((diffX == 0) && (diffY > 0)) {
      if (e->getPosition().x == path.at(j).b.x) {
        ...
        betweenAandB = (path.at(j).a.y <= e->getPosition().y) &&
                       (e->getPosition().y < path.at(j).b.y);
      }
    } else if ((diffX > 0) && (diffY == 0)) {
      if (e->getPosition().y == path.at(j).b.y) {
        // Is the x-coordinate of the enemy on the interval [a.x, b.x).
        betweenAandB = (path.at(j).a.x <= e->getPosition().x) &&
                       (e->getPosition().x < path.at(j).b.x);
      }
    } else if ((diffX == 0) && (diffY < 0)) {
      if (e->getPosition().x == path.at(j).b.x) {
        // Is the y-coordinate of the enemy on the interval (b.y, a.y].
        betweenAandB = (path.at(j).b.y < e->getPosition().y) &&
                       (e->getPosition().y <= path.at(j).a.y);
      }
    } else if ((diffX < 0) && (diffY == 0)) {
      if (e->getPosition().y == path.at(j).b.y) {
        // Is the x-coordinate of the enemy on the interval (b.x, a.x].
        betweenAandB = (path.at(j).b.x < e->getPosition().x) &&
                       (e->getPosition().x <= path.at(j).a.x);
      }
    }
...
/**
 * This method is a helper function for calculating new position of the
 * Enemy-instance, when it goes out of the current Vec2D. It returns a pair of
 * new position and the direction of the enemy.
 */
std::pair<Pos, char> advanceEnemyByOut(const Enemy* e, const float out,
                                       const int currIndex,
  ...
  for (; i < (int)path.size(); i++) {
    float distAB = std::abs((path.at(i).b.x - path.at(i).a.x) +
                            (path.at(i).b.y - path.at(i).a.y));

    // If current distance AB is smaller than offset, then remove offset from
    // remaining and continue with the next Vec2D.
    if (distAB < remain) {
      remain -= distAB;
      continue;
    }
...
void Enemy::update() {
  // In case the enemy is alive, advance enemy position by one tick according to
  // the path.
  if (this->isAlive()) {
    ...
    // Tells the direction of the enemy (North - 'N', East - 'E', South - 'S',
    // West - 'W').
    ...
    // vector.
    bool outX = (unitVec1.x > 0) ? (position_.x > currVec.b.x)
                                 : (position_.x < currVec.b.x);
    bool outY = (unitVec1.y > 0) ? (position_.y > currVec.b.y)
                                 : (position_.y < currVec.b.y);

    if (outX || outY) {
      // Check how much enemy goes out.
      float out = (unitVec1.x != 0) ? std::abs(position_.x - currVec.b.x)
                                    : std::abs(position_.y - currVec.b.y);
      std::pair<Pos, char> res =
          advanceEnemyByOut(this, out, currVecIndex, path, game_);
