...
void Cannon::fire() {
    ...
    Pos vecToEnemy = Pos{ e->getPosition().x - position_.x, e->getPosition().y - position_.y };
    float vecToEnemyLen = sqrt(vecToEnemy.x * vecToEnemy.x + vecToEnemy.y * vecToEnemy.y);
    // Find unit vector of vecToEnemy.
    Pos unitVecToEnemy = Pos{ vecToEnemy.x / vecToEnemyLen, vecToEnemy.y / vecToEnemyLen };
